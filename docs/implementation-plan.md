...
- **Service identification convention (ARCH-007)**: every downstream-service identifier attached to an API surface is named consistently across protocols -- REST tags today (`tags=["<service-name>"]`, e.g. `gateway-api`'s `runs.router` tagged `"validation-service"`), and the equivalent naming for gRPC/GraphQL if/when either is introduced (still **not** in use -- see section 4). Full convention text, including the gRPC `.proto` package-naming rule and the GraphQL type-prefix rule, lives in [../services/gateway-api/README.md](../services/gateway-api/README.md) (where the one real REST instance lives) rather than duplicated here.

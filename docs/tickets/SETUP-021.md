...
ticket to one module's files). **Status**: todo. **Priority**: Should.
...
- [ ] Both services attach one additional `RecentErrorsHandler` to the root logger, alongside (not
  replacing) `OPS-006`'s existing formatter/filter.
- [ ] `GET /diagnostics/recent-errors` (operator-authenticated per each service's own existing
  ...
  secret value.
- [ ] `/monitoring` renders both services' recent-error lists, most-recent first, with the buffer's
  disclosed limitations (no cross-restart persistence, no cross-service search) stated in the page
  copy, not silently omitted.
- [ ] No alerting/paging behavior is added anywhere in this ticket — a list on a page only.

## Test acceptance criteria

- [ ] `libs/common/tests/test_diagnostics.py` (new): buffer caps at 50, drops oldest first, only
...
- [ ] `services/gateway-api/tests/test_diagnostics.py` (new): unauthenticated → `401`/`403` per
...
- [ ] `services/dashboard-web/tests/test_operator_diagnostics.py` (new): mirrors the gateway-api test
  shape for this service's own endpoint and gate.
- [ ] `services/dashboard-web/tests/test_monitoring.py` extended: mocks gateway-api's
  `/diagnostics/recent-errors` response and asserts it renders on `/monitoring`.
- [ ] Full test suites for `libs/common`, `services/gateway-api`, `services/dashboard-web` run, zero
  regressions.
...
- [ ] `services/gateway-api/README.md` and `services/dashboard-web/README.md` each get a
...
- [ ] `libs/common`'s README (if one exists covering `naive_first_common`'s exports) notes the new
  `RecentErrorsHandler` export.
- [ ] `docs/product/backlog-first-run-setup-and-ops.md`'s `SETUP-021` acceptance boxes checked, status

...
Acceptance criteria:
- [x] A wrapper class implements the same `Baseline` interface (`predict(train, test) -> Series`) `naive_first_engine.baselines.Naive0`/`NaiveLast` already implement, backed by a client-supplied prediction series instead of a computed forecast.
- [x] `POST /runs` optionally accepts a client-prediction reference alongside the mandatory naive baselines — the naive baselines are never optional or skippable, per naive_first_engine's own structural rule (solution-design.md section 1 principle 1).
- [x] No new execution path is introduced that runs arbitrary client code — this remains "client submits predictions," not "client submits a model," per solution-design.md section 1 principle 2.
- [x] Any report/response surface built on top of a client-supplied prediction run states explicitly that the platform validates the comparison (client series vs. naive baselines, honestly computed), not the provenance of the client's predictions — so "beat Naive0 in this audit" is never presented or read as "this platform certifies your model didn't leak."
...
### VS-022 — `GET /runs` tenant-scoped list endpoint [Must]
**As** `dashboard-web` (via `gateway-api`'s proxy, GW-016) and any other future caller needing to enumerate a tenant's runs without knowing IDs in advance, **I want** a tenant-scoped, paginated `GET /runs` endpoint ordered by `created_at` descending, **so that** `dashboard-web`'s `DASH-005` (currently blocked, flagged as `DASH-005-GAP` in `docs/product/backlog-dashboard-web.md`) has a real endpoint to render a runs list from, and no client is forced to invent local record-keeping to fake this capability.

Acceptance criteria:
- [ ] `GET /runs` accepts optional `limit` (default e.g. 20, max e.g. 100 — a hard server-side ceiling, not merely a client-suggested default) and `offset` (default 0) query params; an out-of-range `limit` (e.g. `<1` or `>100`) returns `422`, not a silently clamped value.
- [ ] Results are scoped to the resolved tenant via the same `Depends(get_tenant_context)` seam VS-007/VS-008 already use — no raw/unfiltered query, no bypass of `ValidationRunRepository`'s tenant-scoping obligation; a cross-tenant listing is structurally impossible (the repository method takes `tenant_id` as its first parameter, per this module's own established convention, `interfaces.py`'s module docstring AC2).
- [ ] `ValidationRunRepository` gains a new method, `list_runs(tenant_id, limit, offset) -> list[RunRecord]`, implemented by both the interim SQLite backend (VS-004) and the Postgres-backed backend (VS-013) — both already exist and are done, so this method is added to both without changing either's existing call sites for `create_run`/`get_run`/`update_run_status`. The Postgres implementation relies on RLS (already enforced at the DB level per INF-014/VS-021) as the isolation mechanism, not an application-level `WHERE tenant_id = ...` alone — consistent with this platform's established defense-in-depth stance.
- [ ] Ordering is `created_at DESC` — the repository's job, not the caller's (mirrors `get_splits`'s own "ordering is this method's responsibility" precedent, `interfaces.py` module docstring).
- [ ] Response shape reuses `naive_first_common.contracts`: a new `RunSummaryResponse` (a strict subset of `RunDetailResponse`'s fields — `id, dataset_id, horizon, status, created_at, completed_at` — no `split_config`/`tenant_id`/`failure_reason` needed for a list view, avoiding over-fetching) is added to `libs/common/src/naive_first_common/contracts.py` as the single canonical definition, imported (not redefined) by both `validation-service`'s router and `gateway-api`'s future proxy (GW-016) — same DRY convention ARCH-003 already established for `RunDetailResponse`/`SplitResultResponse`. Endpoint returns `{items: list[RunSummaryResponse], limit: int, offset: int, total: int}` (or equivalent — exact envelope documented in the README, but `total` must be a real count, not omitted, so a caller can implement "next page" without guessing).
- [ ] An empty tenant (zero runs) returns `200` with an empty `items` list, not `404`.
- [ ] Tests: a tenant with 3 runs sees exactly its own 3, in `created_at DESC` order; a second tenant's runs never appear in the first tenant's results (non-tautological — asserts by id, not just count); `limit`/`offset` paginate correctly across a seeded set larger than one page; `limit=0` or `limit>100` returns `422`.
- [ ] `services/validation-service/README.md`'s Routes section gains `GET /runs`, matching VS-016's doc-sync check (`check_doc_sync.py` must not fail after this addition).

Rationale for priority: Must — this closes `DASH-005-GAP`, a disclosed capability gap blocking `dashboard-web`'s own Must-priority `DASH-005` story; without it, `dashboard-web`'s core PoC loop stays incomplete (a tenant can submit and view a run by ID but can never discover one without external record-keeping). Matches this service's stated "owns: the three contract endpoints" boundary, extended by one, not a new module.
Depends on: VS-003, VS-004, VS-013 (all done)

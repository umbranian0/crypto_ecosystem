...
8. **For a bug-fix ticket touching a live-traffic path, reproduce the original failure against the real running stack, not just the new regression test.** A passing unit/route test proves the code change is internally consistent; it doesn't prove the fix reaches production. If a live instance of the affected service is reachable (check `docker ps` for this platform's `naive-first-*` containers — see the local-stack-topology note in this repo's own notes if unsure whether a service runs in Docker or as a bare local process), actually hit the endpoint/page that used to fail and confirm it now succeeds, using the real data that triggered the original report where possible (not only synthetic fixtures). If no live instance is reachable, say so explicitly in the report as a disclosed gap rather than silently skipping this step — don't claim "fixed" on test evidence alone when a live check was possible but not done.

9. **End with an explicit go/no-go**, not just a list of findings. If everything genuinely passes, say so plainly rather than manufacturing findings to look thorough — a clean report is a valid, useful outcome.
